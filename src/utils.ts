...
export function asHexColor(value: unknown): string | null {
  const s = asString(value);
...
export function toTrustedEmbedUrl(rawUrl: unknown): string | null {
  ...
  if (YOUTUBE_HOSTS.has(url.hostname)) {
    ...
    if (!validVideoId && !validListId) return null;
    ...
    if (validListId) embed.searchParams.set("list", validListId);
